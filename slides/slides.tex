...
\begin{Thm}{Tangent Space at Identity is a Lie Algebra}
...
\pause
\vspace{0.3em}
\textbf{Note:} $\SqBr{\bm L_i,\bm L_j}=\eps_{ijk}\bm L_k$ —
...
\pause
\vspace{0.3em}
\textbf{Note:} $\SqBr{\bm T_i,\bm T_j}=\eps_{ijk}\bm T_k$ —
...
\pause
\begin{Thm}{Double Cover \texorpdfstring{$\SU(2)\to\SO(3)$}{SU(2)→SO(3)}}
$\Phi:\SU(2)\to\SO(3)$ is:
\begin{enumerate}
\item A smooth group homomorphism: $\Phi(\bm U\bm V)=\Phi(\bm U)\circ\Phi(\bm V)$.
\item Surjective.
...
\begin{Thm}{Lie Algebra Isomorphism \texorpdfstring{$\so(3)\cong\su(2)$}{so(3)≅su(2)}}
